...
type SingleInlineEditWithLabelDefault struct {
	app.Compo
}

func (c *SingleInlineEditWithLabelDefault) Render() app.UI {
	return app.Form().
		Class("pf-c-inline-edit").
		ID("single-inline-edit-with-label-example").
		Body(
			app.Div().
				...
				Body(
					app.Label().
						...
						Body(
							app.Text("Single inline edit group"),
						),
					app.Div().
