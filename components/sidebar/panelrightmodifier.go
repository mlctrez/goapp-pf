...
type PanelRightModifier struct {
	app.Compo
}

func (c *PanelRightModifier) Render() app.UI {
	return app.Div().
		Class("pf-c-sidebar pf-m-panel-right").
		Body(
			app.Div().
						...
						Body(
							app.Div().
								...
								Body(
									app.P().
										Body(
											app.Text("Moves the panel to the right via the modifier class"), app.B().
												Body(
													app.Text(".pf-m-panel-right"),
												),
											app.Text("on the sidebar component."),
										),
									app.P().
										Body(
